...
def EvaluateBoard(board): # Parameter is the board matrix, called "board" throughout code
    ...
    if rows.max() == 3 or cols.max() == 3 or diag1.max() == 3 or diag2.max() == 3:
        return 1
    ...
    if rows.min() == -3 or cols.min() == -3 or diag1.min() == -3 or diag2.min() ==-3:
        return -1
...
def InitNN(BOARD_SIZE): # The board_size value takes the length of one side of the board matrix
...
def Move(ai,board,side):
    ...
    # Baord position from function is placed in the matrix
    board[i,j] = side
...
for loop in range(10000):
    ...
    if result == -1:
        ...
        sizes[2] += 1
plt.pie(sizes,explode=None,labels=labels,colors=['red','blue','gold'],autopct='%1.1f%%',shadow=True)
plt.axis('equal')
plt.show()
print sizes
